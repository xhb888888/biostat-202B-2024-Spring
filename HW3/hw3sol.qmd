...
**Answer:**
\begin{align*}
f_{Y(i),Y(j)}(y,z)&=P(Y_{(i)}=y,Y_{(j)}=z)\\
&=lim_{\Delta y\to \infty, \Delta z\to \infty}\frac{P(\text{i-1 }Y_t's<y, \text{ one }Y_t\in (y,y+\Delta y),\text{ j-i-1 }Y_t's >y+\Delta y \text{ and }<z, \text{ one }Y_t\in (z,z+\Delta z),\text{ n-j } Y_t's >z+\Delta z)}{\Delta y \Delta z}\\
...
Therefore, V and R are independent and $\frac{X_{(1)}}{X_{(n)}} \perp X_{(n)}$
